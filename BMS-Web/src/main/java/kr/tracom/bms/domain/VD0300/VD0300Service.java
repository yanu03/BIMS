...
public class VD0300Service extends ServiceSupport {
	...
	public Map VD0300G3S0() throws Exception {
		int iCnt = 0;
		int uCnt = 0;
		int dCnt = 0;		
		
		List<Map<String, Object>> param = getSimpleList("dlt_BMS_DVC_COND_LOG");
		try {
			for (int i = 0; i < param.size(); i++) {
				Map data = (Map) param.get(i);
				String rowStatus = (String) data.get("rowStatus");
				if (rowStatus.equals("C")) {
					iCnt += vd0300Mapper.VD0300G3I0(data);
				} else if (rowStatus.equals("U")) {
					uCnt += vd0300Mapper.VD0300G3U0(data);
				} else if (rowStatus.equals("D")) {
					dCnt += vd0300Mapper.VD0300G3D0(data);
				} 
			}			
		} catch(Exception e) {
			if (e instanceof DuplicateKeyException)
